  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const length = "4";
        const roundNumber =
          _data[2].status === "success" ? parseInt(_data[2].result) : 0;

        setData({
          tokensSold:
            receiveData[0].status === "success"
              ? parseFloat(
                  formatUnits(receiveData[0].result, global.PROJECT_TOKEN.decimals)
                )
              : 0,
          tokenBalance:
            receiveData[1].status === "success"
              ? parseFloat(
                  formatUnits(receiveData[1].result, global.PROJECT_TOKEN.decimals)
                )
              : 0,
          tokenPrice_denom:
            receiveData[3].status === "success"
              ? parseFloat(
                  formatUnits(receiveData[3].result, global.PROJECT_TOKEN.decimals)
                )
              : 0,
          totalToken:
            receiveData[4].status === "success"
              ? parseFloat(
                  formatUnits(receiveData[4].result, global.PROJECT_TOKEN.decimals)
                )
              : 0,
          ...
          totalSoldAmount:
            _data[0].status === "success"
              ? parseFloat(
                  formatUnits(_data[0].result, global.PROJECT_TOKEN.decimals)
                )
              : 0,
          totalFundsInUSD:
            _data[1].status === "success"
              ? parseFloat(formatUnits(_data[1].result, global.usdDecimals))
              : 0,
          roundNumber,
          currentTokenPrice:
            _data[3].status === "success"
              ? parseFloat(formatUnits(_data[3].result, global.usdDecimals))
              : 0,
          plsAmountFor1USD:
            _data[4].status === "success"
              ? parseFloat(
                  formatUnits(
                    _data[4].result,
                    global.chain.nativeCurrency.decimals
                  )
                )
              : 0,
          nextRoundStartTime:
            _data[5 + roundNumber].status === "success"
              ? parseInt(_data[5 + roundNumber].result)
              : 0,
          ...
          payTokenBalance: address
            ? _data.slice(7 + tRound, 7 + tRound + length)
            : [],
          payTokenAllowance: address
            ? _data.slice(7 + tRound + length, 7 + tRound + 2 * length)
            : [],
          ethBalance,
        });
      } catch (error) {
